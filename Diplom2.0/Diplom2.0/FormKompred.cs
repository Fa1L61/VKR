...
{
    public partial class FormKompred : Form
    {
        public FormKompred()
        {
            InitializeComponent();
        ...
        private void InitializeData()
        {
            ...
            try
            {
                ...
                string Comstr = "Select Zakazi.data_zakaza as 'Дата создания ',Akts.nomer_akt as 'Номер предложения', Zakazi.id_zakaza as 'Номер заказа'," +
                    " Contacts.Name as 'Имя заказчика', Contacts.Surname as 'Фамилия заказчика', Contacts.Otchestvo " +
                    "as 'Отчество заказчика', zakazi.predlojenie as 'Предложение' from Zakazi "+
                    "join Contacts on Contacts.Id_contact = zakazi.Id_contact " +
                    "join Akts on Akts.nomer_zakaza = zakazi.id_zakaza";
